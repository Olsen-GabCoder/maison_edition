...
export class BookService {
  ...
  // === MÉTHODE MODIFIÉE ===
  /**
   * Récupère un livre spécifique par son ID avec logs de débogage et take(1).
  ...
  getBookById(id: number): Observable<Book | undefined> {
    console.log(`[BookService] getBookById(${id}): Début.`);
    // Log la valeur actuelle AVANT même de retourner l'observable
    const currentValue = this.booksSubject.getValue();
    console.log(`[BookService] getBookById(${id}): Valeur ACTUELLE du BehaviorSubject (${currentValue.length} livres).`);

    return this.booksSubject.asObservable().pipe( // Utiliser asObservable()
      take(1), // <<<=== AJOUTÉ : prend seulement la première valeur émise (la valeur actuelle)
      tap(books => {
         // Ce log devrait maintenant apparaître IMMÉDIATEMENT
         console.log(`[BookService] getBookById(${id}): --- DANS LE PIPE --- books$ a émis ${books.length} livres.`);
      }),
      map(books => {
          const found = books.find(book => book.id === id);
          console.log(`[BookService] getBookById(${id}): --- DANS LE PIPE --- map() a trouvé:`, found ? 'Oui' : 'Non');
          return found;
      }),
      tap(foundBook => {
           console.log(`[BookService] getBookById(${id}): --- DANS LE PIPE --- TAP final après map. Valeur émise:`, foundBook);
      })
    );
  }
